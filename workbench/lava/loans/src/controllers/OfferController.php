...
class OfferController extends LoansBaseController {
    ...
    public function injectDataSources() {
        ...
        $this->view_data['dataSource']['user_id'] = $this->callController(\Util::buildNamespace('accounts', 'user', 1), 'getMerchantsHtmlSelect', array($this->merchant['id'], 'id', 'full_name', \Lang::get('common.select')));

        //Get this accounts officer id
        $this->view_data['dataSource']['officer_id'] = $this->callController(\Util::buildNamespace('accounts', 'user', 1), 'getMerchantsHtmlSelect', array($this->merchant['id'], 'id', 'full_name', \Lang::get('common.select')));
