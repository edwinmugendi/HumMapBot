 ...
 */
class VehicleController1 extends AccountsBaseController {
...
//E# afterUpdating() function
    ...
    private function afterCreatingAndUpdating(&$controller_model) {
        ...
        if (!in_array($controller_model->id, $currentVrmIds)) {
            $now = Carbon::now();
            $user_model->vehicles()->attach($controller_model->id, array('created_at' => $now, 'updated_at' => $now));
        }//E# if else statement
        //Get user by token
        $user_model = $this->callController(\Util::buildNamespace('accounts', 'user', 1), 'getModelByField', array('token', $this->input['token'], $parameters));

        //Count user vehicles
        $user_vehicles = $user_model->$relation->count();

        //Save user
        $save_user = false;

        if ($user_vehicles == 1) {//Only one vehicle exists
            //Default this vehicle
            $user_model->vehicle_id = $user_model->vehicles[0]->id;

            //Update save user
            $save_user = true;
        }//E# if statement

        if ($user_vehicles) {//Vehicles exists
            foreach ($user_model->$relation as $single_vehicle) {//Loop via the vehicles
                //Data to update
                $data_to_update = array();

                if ($controller_model->id == $single_vehicle->id) {
                    $data_to_update['dropped_at'] = 'null';
                    //Set is default
                    if (($this->input['is_default'])) {
                        //Default this vehicle
                        $user_model->vehicle_id = $single_vehicle->id;

                        //Update save user
                        $save_user = true;
                    } else {
                        if ($user_model->vehicle_id == $single_vehicle->id) {
                            //Default this vehicle
                            $user_model->vehicle_id = '';
                ...
                if ($data_to_update) {//Update
                    ...
                    //Update pivot
                    $this->updatePivotTable($user_model, $relation, $single_vehicle->id, $data_to_update);
                }//E# if statement
            }//# if else statement
        }//E# if statement
