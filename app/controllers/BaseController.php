...
//E# beforeExportingToCsv() function
     ...
     * Export to CSV
     * 
     * @param boolean $export_indexes_only Export indexes only
     * 
     * @return CSV download or text to show on browser
     */
    public function exportToCsv($export_indexes_only = true) {
        return \Excel::create($this->org['name'] . '-' . $this->layout->title, function($excel) use ($export_indexes_only) {
                    ...
                    if ($export_indexes_only) {
                        //Cache model namespace
                        $model = \Util::buildNamespace($this->package, $this->controller, 2);

                        //Create a model
                        $controller_model = new $model();

                        //Get model keys
                        $model_keys = array_keys($controller_model->viewFields);

                        if ($data_array) {
                            $new_data_array = array();
                            foreach ($data_array as $single_row) {
                                $row = array_only($single_row, $model_keys);
